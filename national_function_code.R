...
Provider_List[["Referrals"]] <- tbl(
  ...
  build_sql(
    ...
    if (Specialty == "X01") {
      ...
      sql(paste0(
        "
                 WHERE SPECIALTY_CODE IN ('",
        str_replace_all("170'', ''172'', ''173", "''", "'"),
        "') "
      ))
    } else {
      sql(paste0("WHERE SPECIALTY_CODE = ", "'", as.character(Specialty), "'"))
    },
    "
...
Provider_List[["NotSeen_Recovered"]] <- tbl(
  ...
  build_sql(
    ...
    if (Specialty == "X01") {
      sql(paste0("
                 WHERE DF.TreatmentSpecialtyCode NOT IN ('", str_replace_all(outpatients_regex, "''", "'"), "') "))
    } else {
      sql(paste0("WHERE DF.TreatmentSpecialtyCode = ", "'", as.character(Specialty), "'"))
    },
